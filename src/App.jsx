...
const disc = b => Math.round(((b.original_price - b.price) / b.original_price) * 100);
...
/* Fallback restaurant images when restaurantImage is empty */
const REST_FALLBACK = [
...
const BAG_HERO = {
  ...
  "Fine Dining":{name:"Chef's Tasting Portion",sub:"Starter + main — restaurant's best",img:"https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=300&q=80"},
...
/* Scrolling food facts banner */
const FACTS = [
  "🌍 1/3 of all food produced globally is wasted every year",
  "🌱 Saving one meal bag prevents ~1.2 kg of CO₂ emissions",
  ...
  "🥗 Buying a ReBite bag saves up to 80% compared to menu price",
  "🌾 40% of India's fruits & vegetables are lost post-harvest",
  "🏙️ Delhi restaurants alone discard thousands of meals every night",
...
const getRestImage = bag => bag.restaurantImage || REST_FALLBACK[bag.id?bag.id.charCodeAt(1)%REST_FALLBACK.length:0];
...
/* ═══════════════════════════════════
   STATIC CSS  (zero JS interpolation)
═══════════════════════════════════ */
const STATIC_CSS = `
...
/* FACTS TICKER */
.ticker-wrap{background:var(--green);border-top:1px solid rgba(255,255,255,.15);padding:5px 0;overflow:hidden;}
.ticker-track{display:flex;gap:0;animation:tickerScroll 40s linear infinite;}
.ticker-track:hover{animation-play-state:paused;}
.ticker-item{white-space:nowrap;font-size:10px;color:rgba(255,255,255,.9);font-family:'DM Sans',sans-serif;padding:0 28px;}
@keyframes tickerScroll{from{transform:translateX(0)}to{transform:translateX(-50%)}}
/* SEARCH + FILTERS — only shown on home tab */
...
.filter-bar{display:flex;align-items:center;gap:6px;padding:6px 14px 4px;overflow-x:auto;scrollbar-width:none;background:var(--bg);}
...
/* Quick CTA chips */
.qchip{padding:6px 11px;border-radius:20px;border:1.5px solid var(--border);background:var(--card);font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;transition:all .2s;color:var(--dark);flex-shrink:0;}
.qchip.active{border-color:var(--orange);background:var(--orange);color:#fff;}
.filter-div{width:1px;height:18px;background:var(--border);flex-shrink:0;}
.results-row{display:flex;align-items:center;justify-content:space-between;padding:2px 14px 2px;}
...
/* DROPDOWN */
.dropdown{position:fixed;background:var(--card);border-radius:16px;box-shadow:0 8px 40px rgba(0,0,0,.25);z-index:9999;overflow:hidden;animation:ddIn .15s ease;border:1px solid var(--border);}
...
/* SCROLL BODY */
.scroll-body{flex:1;overflow-y:auto;padding-bottom:130px;-webkit-overflow-scrolling:touch;scroll-behavior:smooth;}
/* URGENCY BANNER */
...
.cimg-wrap img{width:100%;height:100%;object-fit:cover;transition:transform .3s,opacity .4s;}
.card:hover .cimg-wrap img{transform:scale(1.06);}
.cbadges{position:absolute;top:6px;left:6px;display:flex;flex-direction:column;gap:3px;pointer-events:none;}
...
.scroll-top{position:fixed;bottom:130px;right:calc(50% - var(--w)/2 + 14px);z-index:89;width:36px;height:36px;border-radius:50%;background:var(--card);border:1.5px solid var(--border);box-shadow:0 2px 12px rgba(0,0,0,.15);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:15px;animation:popIn .2s ease;transition:opacity .2s;}
...
/* ADDRESS FIELD in delivery mode */
.addr-wrap{margin-bottom:13px;}
.addr-label{font-family:'Syne',sans-serif;font-size:10px;font-weight:700;color:var(--gray);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;display:block;}
.addr-input{width:100%;background:var(--card);border:1.5px solid var(--border);border-radius:11px;padding:10px 13px;font-family:'DM Sans',sans-serif;font-size:13px;color:var(--dark);outline:none;resize:none;}
.addr-input:focus{border-color:var(--green);}
.addr-shortcuts{display:flex;gap:6px;margin-top:6px;flex-wrap:wrap;}
.addr-chip{background:var(--gp);color:var(--green);border:none;border-radius:20px;padding:4px 10px;font-size:10px;font-weight:700;cursor:pointer;font-family:'Syne',sans-serif;}
...
/* ── Dynamic theme vars (injected at runtime, never processed by Vite CSS) ── */
function ThemeVars({dark}){
  ...
  const v=dark?D:L, bg=dark?"#080D09":"#c8d8c8";
  return <style>{`:root{${Object.entries(v).map(([k,v])=>`${k}:${v}`).join(";")}}body,html{background:${bg}}.shell{background:${bg}}`}</style>;
}
  ...
  useEffect(()=>{
    ...
    const init=()=>{
      ...
      Object.entries(g).forEach(([a,ab])=>{
        ...
        const icon=L.divIcon({html:`<div style="background:#2D6A4F;color:#fff;font-family:sans-serif;font-weight:700;font-size:11px;padding:5px 9px;border-radius:20px;white-space:nowrap;box-shadow:0 2px 8px rgba(0,0,0,.3);">${AREA_ICONS[a]||"📍"} ${a} · ${ab.length}</div>`,className:"",iconAnchor:[40,14]});
...
/* ── Card image that alternates dish/restaurant every 4s ── */
function CardImage({bag}){
  const [showRest,setShowRest]=useState(false);
  useEffect(()=>{
    const t=setInterval(()=>setShowRest(s=>!s),4000+Math.random()*2000);
...
export default function App(){
  ...
  function handleLogout(){setAuthed(false);setPhone("");setPw("");}

  /* ─ enrich bags with restaurant data ─ */
  ...
  /* ─ FILTERING — all conditions applied correctly ─ */
  const filtered=enriched.filter(b=>{
    /* search */
    const q=search.toLowerCase().trim();
    if(q&&!b.restaurantName.toLowerCase().includes(q)&&!b.restaurantArea.toLowerCase().includes(q)&&!(b.type||"").toLowerCase().includes(q)&&!b.restaurantCategory.toLowerCase().includes(q))return false;
    /* area dropdown */
    if(areaFilter!=="All"&&b.restaurantArea!==areaFilter)return false;
    /* type dropdown */
    if(typeFilter!=="All"&&b.type!==typeFilter&&b.restaurantCategory!==typeFilter)return false;
    /* veg toggle */
    ...
    /* quick chip: great offers (>60% off) */
    if(chipGreat&&disc(b)<=60)return false;
    return true;
  }).sort((a,b)=>{
    if(sortBy==="price_asc")return a.price-b.price;
    if(sortBy==="price_desc")return b.price-a.price;
    if(sortBy==="discount")return disc(b)-disc(a);
  ...
  const clearAll=()=>{setAreaFilter("All");setTypeFilter("All");setVegOnly(false);setMaxPrice(500);setSortBy("default");setChip4plus(false);setChipGreat(false);setSearch("");};
  ...
  function openDropdown(name,e){
    ...
    if(l<8)l=8;
    setDdPos({top:btn.bottom+6,left:ar.left+l});
    setOpenDD(openDD===name?null:name);
  }

  function toggleVeg(){
    const n=!vegOnly;setVegOnly(n);
    if(n){setVegBanner(true);setTimeout(()=>setVegBanner(false),3000);}
    else setVegBanner(false);
  }

  /* ─ cart helpers ─ */
  const showToast=m=>{setToast(m);setToastKey(k=>k+1);setTimeout(()=>setToast(null),2200);};
  const addToCart=(bag,mode,time,addr)=>{
  ...
  <div className="shell"><div className="app">
    ...
    <div className="sticky-top" ref={ddRef}>
      ...
      {/* Search + Filters — ONLY on home tab */}
      {tab==="home"&&<>
        <div className="sw">
          <div className="sb">
        ...
        {/* Quick CTA chips */}
        <div className="filter-bar" style={{paddingTop:2,paddingBottom:5}}>
          <span style={{fontSize:9,fontWeight:700,color:"var(--gray)",fontFamily:"Syne",letterSpacing:.5,textTransform:"uppercase",flexShrink:0}}>Quick:</span>
          <button className={`qchip ${chip4plus?"active":""}`} onClick={()=>setChip4plus(v=>!v)}>⭐ 4+ Rating</button>
          <button className={`qchip ${chipGreat?"active":""}`} onClick={()=>setChipGreat(v=>!v)}>🔥 Great Offers</button>
          <button className={`qchip ${vegOnly?"active":""}`} style={vegOnly?{borderColor:"#22C55E",background:"#22C55E"}:{}} onClick={toggleVeg}>🌿 Veg Only</button>
          <button className={`qchip ${areaFilter==="CP"?"active":""}`} onClick={()=>setAreaFilter(a=>a==="CP"?"All":"CP")}>🏛️ CP</button>
          <button className={`qchip ${areaFilter==="Hauz Khas"?"active":""}`} onClick={()=>setAreaFilter(a=>a==="Hauz Khas"?"All":"Hauz Khas")}>🌳 HK</button>
          <button className={`qchip ${sortBy==="discount"?"active":""}`} onClick={()=>setSortBy(s=>s==="discount"?"default":"discount")}>💸 Best Deal</button>
        </div>
    ...
    {openDD&&<div className="dropdown" style={{top:ddPos.top,left:ddPos.left,width:openDD==="price"?240:210}} onClick={e=>e.stopPropagation()}>
      ...
      {openDD==="type"&&<><div className="dd-hd">Bag Type</div>
      ...
      {openDD==="sort"&&<><div className="dd-hd">Sort By</div>
        {[["default","✨ Recommended"],["price_asc","Price: Low → High ↑"],["price_desc","Price: High → Low ↓"],["discount","Biggest Discount 🔥"],["rating","Highest Rated ⭐"]].map(([v,l])=>
          <div key={v} className={`dd-opt ${sortBy===v?"active":""}`} onClick={()=>{setSortBy(v);setOpenDD(null);}}>
            <span>{l}</span><div className="chk">{sortBy===v?"✓":""}</div>
          </div>
        )}
      </>}
      {openDD==="price"&&<><div className="dd-hd">Max Price</div>
        ...
        <div className="dd-footer"><button className="dd-clear-btn" onClick={()=>{setMaxPrice(500);setOpenDD(null);}}>Reset ₹500</button></div>
      </>}
    </div>}
    ...
    >
      ...
      {tab==="home"&&<>
        ...
        {loading
          ?<div className="loading"><span className="spin">🌿</span><p style={{marginTop:10,fontFamily:"Syne",fontWeight:700,fontSize:14,color:"var(--dark)"}}>Loading today's bags...</p></div>
          :filtered.length===0
            ?<div className="empty"><div className="big">😔</div><p>No bags found</p><span style={{fontSize:12}}>Try different filters</span>{hasFilters&&<div onClick={clearAll} style={{marginTop:12,color:"var(--green)",fontWeight:700,fontSize:13,cursor:"pointer"}}>Clear all filters →</div>}</div>
            :<div className="grid">
              {filtered.map(bag=>{
                const d=disc(bag), veg=isVeg(bag);
                return(
                  <div key={bag.id} className="card" onClick={()=>{setSelectedBag(bag);setDetailMode("pickup");setSelectedTime(bag.pickup_start);setDeliveryAddr("");}}>
                    <CardImage bag={bag}/>
                    <div className="cbadges">
                      {d>=50&&bag.quantity>0&&<span className="cbadge">🔥 {d}%</span>}
                    ...
                    <div className="cbody">
                      ...
                      <div className="cbot">
                        ...
                        <button className="add-btn" disabled={!bag.quantity} onClick={e=>{e.stopPropagation();addToCart(bag,"pickup",bag.pickup_start,"");}}>+</button>
                      </div>
                    </div>
                  </div>
                );
      ...
      {/* EXPLORE TAB — no search/filters header */}
      {tab==="explore"&&<div className="page">
        <div className="ptitle">Explore Delhi 🗺️</div>
        <div className="psub">Tap a marker or area card to filter bags</div>
      ...
      {/* ORDERS TAB — no search/filters header */}
      {tab==="orders"&&<div className="page">
        <div className="ptitle">Your Orders 📦</div>
        <div className="psub">Your ReBite history</div>
        {orders.length===0
          ?<div className="empty"><div className="big">📦</div><p>No orders yet</p><span style={{fontSize:12}}>Reserve a bag!</span></div>
          :orders.map(o=>(
            <div key={o.id} className="ocard">
              ...
              <div className="odiv"/>
              <div className="obot">
                <div className="oprice">₹{o.price} <span style={{fontSize:10,color:"var(--gl)",fontWeight:600}}>saved ₹{o.savings}</span></div>
      ...
      {/* PROFILE TAB — no search/filters header */}
      {tab==="profile"&&<div className="page">
        ...
        <button className="lo-btn" onClick={handleLogout}>Log Out</button>
      </div>}
    </div>
    ...
    {cart.length>0&&<div className="fab">
      <button className="fab-btn" onClick={()=>setShowCart(true)}>
        ...
        <span>View Bag</span>
        <span style={{opacity:.8,fontWeight:500,fontSize:12}}>· ₹{cartTotal}</span>
      </button>
    </div>}
    ...
    <div className="bnav">
      {[{id:"home",icon:"🏠",label:"Home"},{id:"explore",icon:"🗺️",label:"Explore"},{id:"orders",icon:"📦",label:"Orders"},{id:"profile",icon:"👤",label:"Profile"}].map(n=>(
        <div key={n.id} className={`nitem ${tab===n.id?"active":""}`} onClick={()=>setTab(n.id)}>
        ...
        {cart.length===0
          ...
          :<>
            {cart.map(i=>(
              <div key={i.id} className="ci">
                ...
                <div className="ci-info">
                  ...
                  <div className="ci-meta">{i.mode==="delivery"?"🛵":"🏃"} {i.pickedTime} · {i.type}{i.deliveryAddr?" · "+i.deliveryAddr:""}</div>
                </div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",margin:"8px 0 4px"}}>
              ...
              <span style={{fontFamily:"Syne",fontWeight:800,fontSize:21,color:"var(--green)"}}>₹{cartTotal}</span>
            </div>
    ...
    {/* ── DETAIL VIEW ── */}
    {selectedBag&&(()=>{
      const di=getDishImage(selectedBag), d=disc(selectedBag);
      ...
      return(
        <div className="detail">
          ...
          {/* Restaurant strip with actual image */}
          <div className="rest-strip">
            <img className="rest-strip-img" src={restImg} alt={selectedBag.restaurantName}
              onError={e=>{e.target.onerror=null;e.target.style.display="none";}}/>
            <div>
              <div className="rest-name">{selectedBag.restaurantName}</div>
              <div className="rest-meta">{selectedBag.restaurantCategory} · {selectedBag.restaurantArea} · ⭐{selectedBag.restaurantRating}<br/>{selectedBag.restaurantDesc?.slice(0,70)}…</div>
            </div>
          </div>
          ...
          <div className="dbody">
            ...
            <div className="dcat">
              ...
              <span>·</span><span>{d}% off · saves ₹{selectedBag.original_price-selectedBag.price}</span>
            </div>
            ...
            {detailMode==="delivery"&&<div className="addr-wrap">
              ...
              <div className="addr-shortcuts">
                {["Home","Work","Other"].map(s=>(
                  <button key={s} className="addr-chip" onClick={()=>setDeliveryAddr(v=>v?v:s+" — ")}>📌 {s}</button>
                ))}
              </div>
            </div>}
            ...
            <div className="bag-contents">
              ...
              <div className="bag-item">
                <div style={{width:52,height:52,borderRadius:10,background:"var(--gp)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>🎲</div>
                <div><div className="bag-item-name">Surprise Item</div><div className="bag-item-sub">A little extra from the kitchen — changes daily!</div></div>
              </div>
            </div>
          ...
          <div className="dfooter">
            ...
            >
              {!selectedBag.quantity?"Sold Out":detailMode==="delivery"&&!deliveryAddr.trim()?"Add address first 📍":detailMode==="delivery"?"Reserve · Delivery 🛵":`Reserve · ${selectedTime} 🏃`}
            </button>
          </div>
        </div>
      );
